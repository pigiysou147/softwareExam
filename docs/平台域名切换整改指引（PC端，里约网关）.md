...
### 方案概述
- **目标**：在并行期同时提供旧域名与新域名访问能力，使用“中转页”友好提示停用日期并在 5 秒后自动跳转至新域名，最终下线旧域名。
- **适用范围**：页面级入口（收藏、门户入口、专题入口等）。不建议对接口/静态资源返回中转页，避免影响程序调用与协议一致性。

### 网关与路由建议
- 旧域名 Host：`fzh.ywtg.foshan.gov.cn`
- 新域名 Host：`yzhcs.fszsj.fs.gdgov.cn`
- 路由策略：
  - 旧域名 `GET /`（或 `/index.html` 等页面入口）返回中转页（HTTP 200），展示停用信息与新域名地址，5s 后自动跳转；
  - 旧域名下的接口路径（如 `/api/**`, `/auth/**`, `/sso/**`, `/ws/**`）在过渡期保持可用，或按计划逐步 301/308 到新域名同路径（灰度，避免影响未改造的第三方）；
  - 严禁对接口返回中转页 HTML；接口必须保持 JSON/二进制协议；
  - 防循环：仅旧域名侧返回中转页；新域名绝不回跳旧域名；
  - SEO：可选择对搜索引擎使用 301 到新域名，或保留中转页并加 `noindex`，二者择一即可。

### 中转页实现要点
- 提示信息：明确旧域名停用日期、新域名可访问地址，并提供立即跳转链接；
- 自动跳转：5 秒倒计时后 `location.replace()` 跳转到新域名，尽量保留原路径、查询与哈希；
- 兜底：`<noscript>` 内使用 `meta refresh`（无法保留路径）；
- 缓存：通过网关/服务端设置 `Cache-Control: no-store, no-cache, must-revalidate`；
- SEO：`<link rel="canonical" href="https://yzhcs.fszsj.fs.gdgov.cn">` 与 `meta name="robots" content="noindex,nofollow"`；
- 示例（完整文件见 `docs/中转页示例.html`）：

```html
<script>
  const NEW_ORIGIN = 'https://yzhcs.fszsj.fs.gdgov.cn';
  const target = NEW_ORIGIN + location.pathname + location.search + location.hash;
  setTimeout(() => location.replace(target), 5000);
</script>
```

### 发布与灰度
- 启用中转页后，监控旧域名的 PV/UV、接口请求量与错误率；
- 分阶段将旧域名接口按路径逐步 301/308 至新域名或直接下线；
- 明确公告停用日期，并在中转页显著位置展示；
- 停用日：停止旧域名 DNS 解析或仅保留 301 至新域名 7–14 天，随后下线。
...
### 范围
- 平台后台配置的所有“外部链接”类入口：侧边菜单、顶部导航、快捷入口、门户卡片、轮播/公告内链、页脚链接、站内跳转配置等。

### 规范与要求
- **一律去硬编码域名**：
  - 同域资源与页面：使用以 `/` 开头的相对路径，例如：`/sso/login`、`/feature/list`、`/doc?id=123`；
  - 通过网关代理的内部系统：按网关约定的前缀（示例 `/gw/xxx` 或 `/proxy/xxx`）配置相对路径，保持同域；
  - 确需跳转到平台新域名的完整页面：使用 `https://yzhcs.fszsj.fs.gdgov.cn/...`，并建议 `target="_blank" rel="noopener"`；
  - 禁止保留旧域名 `https://fzh.ywtg.foshan.gov.cn/...`。
- **路径书写**：优先使用以 `/` 开头的站点绝对路径，避免省略斜杠导致相对当前页面目录的错误跳转；
- **富文本与资源**：审查富文本中的 `href`/`src`（图片、样式、脚本、iframe）是否含旧域名，统一替换；
- **白名单与回调**：如外链跳转到第三方域，确认在后端/网关白名单内，涉及 SSO 回跳的必须同步更新回调 URI。

### 批量排查建议（模板）
- 后台导出配置（菜单/导航/门户等），检索是否包含旧域名：

```sql
-- 表名/字段仅为示例，请以实际库表为准
SELECT id, name, url
FROM   sys_menu
WHERE  url LIKE '%fzh.ywtg.foshan.gov.cn%';

SELECT id, title, link
FROM   cms_portal_card
WHERE  link LIKE '%fzh.ywtg.foshan.gov.cn%';
```

- 若后台配置支持校验，建议新增拦截规则：禁止保存包含旧域名的链接，或给出强提示。

### 变更流程
- 预发环境先改先验：备份配置 -> 替换为相对路径或新域名 -> 全量回归；
- 生产灰度：分批发布导航/菜单配置，观察埋点点击跳转成功率与错误日志；
- 变更记录：记录变更人、时间与项目，便于回溯。

### 常见问题
- 菜单外链仍指向旧域名：点击后跨域或 404；处理为相对路径或新域名；
- 链接未以 `/` 开头：在某些子路径页面变成相对目录跳转，导致 404；
- 跳转到第三方后登录态丢失：确认是否需要通过网关同域代理或更新回调白名单；
- `target="_blank"` 新开页后返回路径不一致：在新域名上保留相同路径/路由以保证兼容。
